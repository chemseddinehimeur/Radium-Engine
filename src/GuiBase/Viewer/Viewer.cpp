...
void Gui::Viewer::mousePressEvent( QMouseEvent* event ) {
         ...
         isKeyPressed( keyMap->getKeyFromAction( Gui::KeyMappingManager::VIEWER_RAYCAST_QUERY ) ) )
    {
        ...
        //        auto r = m_camera->getCamera()->getRayFromScreen( Core::Vector2( event->x(),
        //        event->y() ) ); RA_DISPLAY_POINT( r.origin(), Color::Cyan(), 0.1f );
        //        RA_DISPLAY_RAY( r, Color::Yellow() );
        //        auto ents =
        //        Engine::RadiumEngine::getInstance()->getEntityManager()->getEntities(); for ( auto
        //        e : ents )
        //        {
        //            e->rayCastQuery( r );
        //        }
